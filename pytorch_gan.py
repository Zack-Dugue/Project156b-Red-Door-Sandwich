...
def prepare_dataset():
    ...
    # for now just replace all the nans with -1, we can change this to use nan mask later
    classesdf = traindf[classes].fillna(0)

    # Get the paths to all the images and put it as a list
    train_paths = traindf["Path"].tolist()[:-1]

    print("Initializing Xtrain ...")
    count = 0
    X_train = np.array([np.asarray(Image.open(prefix+path).resize((xsize, ysize))) for path in train_paths])
